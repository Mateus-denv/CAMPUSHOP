...
public class ChatService {
        ...
        public List<ChatPedidoResponse> listarPedidosParaChat(String emailUsuario) {
                ...
                pedidoRepository.findByCompradorEmail(emailUsuario)
                                .forEach(pedido -> pedidos.add(toChatPedidoResponse(pedido, usuarioLogado, false)));

                pedidoRepository.findByVendedorEmail(emailUsuario)
                                .forEach(pedido -> pedidos.add(toChatPedidoResponse(pedido, usuarioLogado, true)));
        ...
        private void validarChatDisponivel(Pedido pedido, Boolean aceitouAviso) {
                ...
                if ("rejeitado".equals(statusNormalizado) || "invalido".equals(statusNormalizado)
                                || "entregue".equals(statusNormalizado)) {
                        throw new RuntimeException(
                                        "O chat para este pedido não está disponível porque o pedido foi rejeitado, invalidado ou já foi entregue.");
                }

                if (!"aceito".equals(statusNormalizado)) {
                        throw new RuntimeException(
                                        "O chat para este pedido ainda não foi liberado. Aguarde a aprovação do vendedor.");
                }

                if (aceitouAviso == null || !aceitouAviso) {
                        throw new RuntimeException(
                                        "É necessário confirmar que seguirá as orientações de segurança clicando em OK no aviso antes de enviar mensagens.");
                }
        }
        ...
        private ChatPedidoResponse toChatPedidoResponse(Pedido pedido, Usuario usuarioLogado, boolean souVendedor) {
                ...
                ChatMensagem ultimaMensagem = mensagensPedido.isEmpty() ? null
                                : mensagensPedido.get(mensagensPedido.size() - 1);
                ...
                return new ChatPedidoResponse(
                                ...
                                ultimaMensagem != null && ultimaMensagem.getCriadoEm() != null
                                                ? ultimaMensagem.getCriadoEm().toString()
                                                : null);
        }
