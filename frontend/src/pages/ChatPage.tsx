...
export function ChatPage() {
  ...
  const [pedidoSelecionado, setPedidoSelecionado] =
    useState<ChatPedidoAPI | null>(null);
  ...
  useEffect(() => {
    ...
    const carregarMensagens = async () => {
      ...
      try {
        const response = await chatAPI.listarMensagens(
  ...
  const enviarMensagem = async () => {
    ...
    if (!aceitouAviso) {
      setBloqueioChat(
        "Você precisa confirmar o aviso de segurança antes de enviar mensagens.",
      );
      return;
    }

    try {
      const response = await chatAPI.enviarMensagem(
        pedidoSelecionado.pedidoId,
        texto.trim(),
        aceitouAviso,
      );
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between gap-2">
                    ...
                    <p className="text-[11px] font-semibold text-slate-500">
                      {pedido.ultimaMensagemData
                        ? new Date(pedido.ultimaMensagemData).toLocaleString(
                            "pt-BR",
                          )
                        : "Sem mensagens"}
                    </p>
          ...
          {!pedidoSelecionado ? (
            <div className="flex min-h-[320px] flex-col items-center justify-center gap-3 text-center text-slate-500">
              ...
              <p className="max-w-xl text-sm leading-6">
                Aqui você poderá trocar mensagens com o vendedor ou comprador
            ...
            <>
              <div className="mb-6 flex flex-col gap-1 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-400">
                    Pedido #{pedidoSelecionado.pedidoId}
                  </p>
                  <h1 className="text-3xl font-black text-slate-900">
                    {pedidoSelecionado.parceiroNome}
                  </h1>
                  <p className="text-sm text-slate-500">
                    {pedidoSelecionado.produtoNome}
                  </p>
                </div>
              ...
              <div className="flex min-h-[420px] flex-col gap-4 rounded-[2rem] border border-slate-200 bg-slate-50 p-4">
                ...
                {carregando && mensagens.length === 0 ? (
                  ...
                  <p className="text-sm text-slate-500">
                    Nenhuma mensagem neste pedido. Envie a primeira mensagem.
                  </p>
                ) : (
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
                <textarea
                  ...
                  placeholder={
                    pedidoSelecionado.status === "aceito"
                      ? "Digite sua mensagem..."
                      : "O chat ficará disponível após a aprovação do pedido."
                  }
                ...
                <button
                  ...
                  disabled={
                    !texto.trim() || pedidoSelecionado.status !== "aceito"
                  }
                >
                  Enviar
                </button>
