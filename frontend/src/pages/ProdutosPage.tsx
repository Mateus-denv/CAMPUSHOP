...
import { CheckCircle2, Heart, Lock, Search, Star } from 'lucide-react'
...
export function ProdutosPage() {
  ...
  const disponibilidadeLabel = (estoque: number) => {
    if (estoque === 0) return 'Indisponível'
    if (estoque <= 2) return 'Últimas unidades'
    return 'Em estoque'
  }

  const disponibilidadeStyle = (estoque: number) => {
    if (estoque === 0) return 'bg-slate-100 text-slate-500'
    if (estoque <= 2) return 'bg-amber-100 text-amber-800'
    return 'bg-emerald-100 text-emerald-700'
  }

  const buildPlaceholderImage = (produto: Produto) => {
    const text = encodeURIComponent(produto.nomeProduto || 'Produto')
    return `https://placehold.co/420x300?text=${text}&font=inter&bg=ffffff&txtclr=0f172a`
  }
    ...
    return produtos
      .filter((produto) => {
        ...
        const correspondeBusca =
          ...
          produto.local?.toLowerCase().includes(termo)

        const correspondeCategoria = categoriaFiltro === 'Todos' || produto.categoria === categoriaFiltro
        const correspondeCampus = campusFiltro === 'Todos' || produto.local === campusFiltro
        const correspondeDisponibilidade =
          disponibilidadeFiltro === 'Todos' ||
          (disponibilidadeFiltro === 'Em estoque' && produto.estoque > 0) ||
          (disponibilidadeFiltro === 'Últimas unidades' && produto.estoque > 0 && produto.estoque <= 2) ||
          (disponibilidadeFiltro === 'Indisponível' && produto.estoque === 0)

        return correspondeBusca && correspondeCategoria && correspondeCampus && correspondeDisponibilidade
      })
      .sort((a, b) => (a.estoque === 0 ? 1 : b.estoque === 0 ? -1 : b.estoque - a.estoque))
  }, [busca, categoriaFiltro, campusFiltro, disponibilidadeFiltro, produtos])

  const categoriasDisponiveis = useMemo(
    () => ['Todos', ...Array.from(new Set(produtos.map((produto) => produto.categoria || 'Marketplace')))],
    [produtos]
  )

  const campusDisponiveis = useMemo(
    () => ['Todos', ...Array.from(new Set(produtos.map((produto) => produto.local || 'Campus')))],
    [produtos]
  )
    ...
    try {
      ...
      const produtosNormalizados: Produto[] = (response.data ?? []).map((produto: any) => ({
        ...
        vendedorNome:
          ...
          '',
        categoria: produto.categoria ?? produto.categoriaNome ?? 'Marketplace',
        local: produto.local ?? produto.campus ?? produto.unidade ?? 'Campus',
        instituicao:
          produto.instituicao ?? produto.instituicaoVendedor ?? produto.usuario?.instituicao ?? produto.local ?? 'Campus',
        avaliacao: Number((4.3 + (Number(produto.idProduto ?? produto.id) % 5) * 0.15).toFixed(1)),
        imagem: produto.imagem ?? produto.foto ?? buildPlaceholderImage({
          idProduto: Number(produto.idProduto ?? produto.id),
          nomeProduto: produto.nomeProduto ?? produto.nome ?? '',
          descricao: produto.descricao ?? '',
          preco: Number(produto.preco ?? 0),
