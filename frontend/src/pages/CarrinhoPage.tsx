    ...
    <Layout>
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        <div className="mb-6">
          ...
          {mensagem && <p data-testid="cart-success-message" className="mt-3 rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-700">{mensagem}</p>}
        </div>

        {cartWithProducts.length === 0 && (
          <div className="rounded-[1.5rem] border border-slate-200 p-6 text-center">
            <p data-testid="cart-empty-message" className="font-semibold text-slate-700">Seu carrinho está vazio.</p>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-4">
              {cartWithProducts.map(({ productId, product: p, quantidade: q }) => (
                <div key={productId} data-testid={`cart-item-${productId}`} className="flex flex-col gap-4 rounded-[1.5rem] border border-slate-200 p-4 sm:flex-row sm:items-center">
            ...
            <div className="space-y-4">
              <div className="rounded-[1.5rem] border border-slate-200 p-5 shadow-sm">
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-200 pt-4">
                  ...
                  <strong data-testid="cart-total" className="text-xl text-blue-700">R$ {total.toFixed(2)}</strong>
                </div>
                <button data-testid="cart-finalize-button" onClick={finalizarPedido} className="mt-4 w-full rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-700 py-3.5 font-semibold text-white shadow-lg shadow-blue-600/20 transition-transform hover:scale-[1.01]">Finalizar pedido</button>
