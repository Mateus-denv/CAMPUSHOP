...
import { cacheProduct, checkoutCart, getCachedProduct, getCart, removeFromCart, updateCartItem } from '@/lib/shop-storage'
...
export function CarrinhoPage() {
  ...
  useEffect(() => {
    ...
    cart.forEach((item) => {
      ...
      if (typeof apiProduct.estoque === 'number' && apiProduct.estoque !== cached.estoque) {
        cacheProduct({
          ...cached,
  ...
  useEffect(() => {
    cart.forEach((item) => {
      const apiProduct = produtosApi.find((produto) => produto.idProduto === item.productId)
      const estoque = apiProduct?.estoque
      if (typeof estoque === 'number' && item.quantidade > estoque) {
        updateCartItem(item.productId, estoque)
      }
    })
    setCart(getCart())
  }, [produtosApi])
  ...
  const atualizarQuantidade = (productId: number, quantidade: number) => {
    const apiProduct = produtosApi.find((produto) => produto.idProduto === productId)
    const estoque = apiProduct?.estoque
    const quantidadeFinal = typeof estoque === 'number' ? Math.min(quantidade, estoque) : quantidade
    updateCartItem(productId, quantidadeFinal)
    setCart(getCart())
  }
  ...
  const finalizarPedido = async () => {
    const itemSemEstoque = cartWithProducts.find((item) => {
      const apiProduct = produtosApi.find((produto) => produto.idProduto === item.productId)
      const estoque = apiProduct?.estoque
      return typeof estoque === 'number' && item.quantidade > estoque
    })
    if (itemSemEstoque) {
      setMensagem('Seu carrinho tem itens acima do estoque disponível.')
      return
    }
    // Assume payment type 1 for now
    const { order, error } = await checkoutCart(1)
    if (!order) {
      setMensagem(error ?? 'Erro ao finalizar pedido. Verifique o estoque.')
      return
    }
    setMensagem(`Pedido ${order.id} criado com sucesso!`)
    setCart(getCart())
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {cartWithProducts.map(({ productId, product: p, quantidade: q }) => {
              ...
              <div key={productId} className="flex flex-col gap-4 rounded-[1.5rem] border border-slate-200 p-4 sm:flex-row sm:items-center">
                ...
                <div className="flex flex-col items-start gap-2 self-start sm:self-center">
                  ...
                  <button
                    onClick={() => atualizarQuantidade(productId, q + 1)}
                    disabled={typeof estoque === 'number' && q >= estoque}
                    title={typeof estoque === 'number' && q >= estoque ? 'Estoque máximo atingido' : undefined}
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-lg text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    +
                  </button>
                  ...
                  {typeof estoque === 'number' && q >= estoque && (
                    <span className="text-xs font-semibold text-red-600">Estoque máximo atingido</span>
                  )}
                </div>
              </div>
              )
